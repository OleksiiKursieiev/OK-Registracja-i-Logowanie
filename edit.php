...
if(!($conn = mysqli_connect($server, $user, $pass, $db)
    ...
    if (isset($_POST['submit']) && isset($_POST['id_wia'])) {
        ...
        $temat = $_POST['temat'];
        $tresc = $_POST['tresc'];
        $sql = "UPDATE wiadomości SET temat='$temat', tresc='$tresc' WHERE wiadomości.id_wia = $id;";
...
<form id="form" action="edit.php" method="post">
        ...
        <div>
            ...
            <input type="text" id="temat" name="temat" value="<?php echo $item['temat'] ?? ''; ?>">
        </div>
        <div>
           <label for="tresc">Tresc: </label>
           <textarea id="tresc" name="tresc"  rows="10" cols="100"><?php echo $item['tresc']; ?></textarea>
        </div>
        <button id="submit" type="submit" name="submit">Zmodyfikuj</button>
</form>
<?php
}
